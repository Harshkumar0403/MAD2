...
class register(Resource):
    def post(self):
        ...
        if role == 'sponsor':
            ...
            db.session.commit()
            return make_response(jsonify({"message": "Sponsor created successfully","user_id":user.id,"sponsor_id":sponsor.sponsor_id, "email": user.email,"username":user.username,"company_name": sponsor.company_name,"industry":sponsor.industry,"budget":sponsor.budget, "token": token, "role": role}), 201)
